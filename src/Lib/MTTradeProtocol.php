...
class MTTradeProtocol
{
    ...
    public function TradeBalance($login, $type, $balance, $comment, &$ticket = null, $margin_check=true)
    {
        ...
        if (!$this->m_connect->Send(MTProtocolConsts::WEB_CMD_TRADE_BALANCE, $data)) {
            if (MTLogger::getIsWriteLog()) {
                MTLogger::write(MTLoggerType::ERROR, 'send trade balance failed');
            }
        ...
        if (($answer = $this->m_connect->Read()) == null) {
            if (MTLogger::getIsWriteLog()) {
                MTLogger::write(MTLoggerType::ERROR, 'answer trade balance is empty');
            }
            return MTRetCode::MT_RET_ERR_NETWORK;
        }
        //--- parse answer
        ...
        if (($error_code = $this->Parse($answer, $trade_answer)) != MTRetCode::MT_RET_OK) {
            if (MTLogger::getIsWriteLog()) {
                MTLogger::write(MTLoggerType::ERROR, 'parse trade balance failed: [' . $error_code . ']' . MTRetCode::GetError($error_code));
            }
            return $error_code;
        }
        //---
    ...
    private function Parse(&$answer, &$trade_answer)
    {
        ...
        if ($command_real != MTProtocolConsts::WEB_CMD_TRADE_BALANCE) {
        ...
        //--- check ret code
        if (($ret_code = MTConnect::GetRetCode($trade_answer->RetCode)) != MTRetCode::MT_RET_OK) {
            return $ret_code;
        }
        //---
        return MTRetCode::MT_RET_OK;
