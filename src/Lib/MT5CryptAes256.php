...
class MT5CryptAes256
{
    ...
    public function Init($aes_key, $len=32)
    {
        ...
        if ($this->m_nk != 4 && $this->m_nk != 6 && $this->m_nk != 8) {
            return '';
        } //("Key is " . ($this->m_nk * 32) . " bits long. *not* 128, 192, or 256.");
        $this->m_nr           = $this->m_nk + self::$m_nb + 2;
    ...
    private function KeyExpansion($key)
    {
      ...
      $this->m_cipher[$i] += ord($key[4 * $i + 1]);
        ...
        for (; $i < self::$m_nb * ($this->m_nr + 1); $i++) {
            ...
            //---
            if ($i % $this->m_nk == 0) {
                $temp = $this->subWord($this->rotWord($temp)) ^ $rcon[$i / $this->m_nk];
            } elseif ($this->m_nk > 6 && $i % $this->m_nk == 4) {
    ...
    private function invShiftRows()
    {
        ...
        for ($i = 1; $i < 4; $i++) {
            for ($j = 0; $j < self::$m_nb; $j++) {
                $temp[($i + $j) % self::$m_nb] = $this->m_cipher_table[$i][$j];
            }
            for ($j = 0; $j < self::$m_nb; $j++) {
                $this->m_cipher_table[$i][$j] = $temp[$j];
            }
        }
    }
    ...
    private function invSubBytes()
    {
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$m_nb; $j++) {
                $this->m_cipher_table[$i][$j] = self::$m_inv_sbox[$this->m_cipher_table[$i][$j]];
            }
        }
    }
    ...
    private function subBytes()
    {
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$m_nb; $j++) {
                $this->m_cipher_table[$i][$j] = self::$m_sbox[$this->m_cipher_table[$i][$j]];
            }
        }
    }
